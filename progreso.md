...
## 🚀 Hitos Recientes
* **Control de Subrogaciones y Estado de Cuentas (Julio 2026):** Se añadió el campo `is_active` en la tabla `users` para controlar el estado de actividad de las cuentas durante periodos de vacaciones o licencia. Se habilitaron los roles `Tesorero Subrogante`, `Prefecto/a Subrogante` y `Gestor de Tesorería Subrogante` en `RoleSeeder.php` y `PolizaController.php`. Cuando una autoridad titular es inactivada, el sistema bloquea su inicio de sesión (`LoginRequest`) y la firma de oficios/renovaciones, permitiendo que únicamente el subrogante activo opere en el sistema.
* **Configuración Dinámica de Firmantes en Oficios (Julio 2026):** Se implementó la gestión dinámica de autoridades desde la sección de Configuración (`ConfiguracionController`, `Configuraciones/Index.vue`). La plantilla de oficios (`oficio_renovacion.blade.php`) ahora toma automáticamente nombres, cargos y correos desde la base de datos sin necesidad de modificar código fuente.
